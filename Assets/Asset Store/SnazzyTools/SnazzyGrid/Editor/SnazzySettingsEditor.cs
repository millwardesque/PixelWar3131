...
public class SnazzySettingsEditor : EditorWindow {
	...
	public SnazzyToolsEditor snazzyToolsEditor;
	...
	void OnGUI()
	{
		...
		scrollPosition = GUI.BeginScrollView(new Rect(0,0,Screen.width,Screen.height), scrollPosition, new Rect(0,0,394+6,1135+6+81));
		EditorGUI.DrawPreviewTexture (new Rect(0,-1,428,1224),SnazzySettingsBGImage);		
		...
		pos.y += 20;
		KeyField (pos,"  Selection next",ref snazzy.selectionNextKey,ref snazzy.selectionNextKeySpecial,27);
		pos.y += 20;
		KeyField (pos,"  Selection previous",ref snazzy.selectionPreviousKey,ref snazzy.selectionPreviousKeySpecial,28);
		pos.y += 20;
//		KeyField (pos,"Unparent & delete",ref snazzy.unparentDeleteKey,ref snazzy.unparentDeleteKeySpecial,27);
		...
		if (snazzyGridRenderer != null) {
			...
			GUI.Label(GetRect5(pos),"Grey Colors");
			GUI.changed = false;
			snazzy.greyScale = GUI.Toggle(GetRect6(pos),snazzy.greyScale,"");
			if (GUI.changed) {
				if (snazzyToolsEditor != null) {
					snazzyToolsEditor.LoadButtons();
					snazzyToolsEditor.Repaint ();
				}
			}
			pos.y += 20;

			GUI.Label (GetRect5(pos),"Selection list length");
			snazzy.selectionListLength = EditorGUI.IntField(GetRect6(pos),snazzy.selectionListLength);
			if (snazzy.selectionListLength > 50) snazzy.selectionListLength = 50;
			else if (snazzy.selectionListLength < 5) snazzy.selectionListLength = 5;
			pos.y += 20;
			...
			bool toggle;
			if (snazzy.snapOffset == 0) toggle = true; else toggle = false;
			GUI.changed = false;
			if (GUI.Toggle (GetRect6(pos),toggle,"")) snazzy.snapOffset = 0f; else snazzy.snapOffset = 0.5f;
	...
	void LoadText()
	{
		string path = SnazzyToolsEditor.installPath+"/Sources/GUI/";
